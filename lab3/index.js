        ...
        if (multiStopPattern.test(result.command)) {
          ...
          if (firstNumber < secondNumber) {
            for (let i = firstNumber; i <= secondNumber; i++) {
              ...
              console.log(
                `Program 2: Worker with id:${Number(i) + 1} has been suspended!`
              );
            }
          }
...
const runThirdProgramThreads = () => {
  let numberOfThreads = 10;
  const workersArray = [];

  for (let i = 1; i <= 10; i += 1) {
    const worker = new Worker('./program3.js', {
      workerData: {
        suspended: true,
        id: i === 10 ? 0 : i,
      },
        ...
        if (multiStopPattern.test(result.command)) {
          ...
          if (firstNumber < secondNumber) {
            for (let i = firstNumber; i <= secondNumber; i++) {
              workersArray[i].postMessage({ type: 'suspend' });
              console.log(
                `Program 4: Worker with id:${Number(i) + 1} has been suspended!`
              );
            }
          }
        } else if (singleStopPattern.test(result.command)) {
          ...
          if (firstNumber < secondNumber) {
            for (let i = firstNumber; i <= secondNumber; i++) {
              workersArray[i].postMessage({ type: 'resume' });
              console.log(
                `Program 4: Worker with id:${Number(i) + 1} has been resumed!`
              );
            }
          }
        } else if (singleStartPattern.test(result.command)) {
